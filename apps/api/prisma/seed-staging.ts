    ...
    for (const [lessonIndex, lesson] of spec.lessons.entries()) {
      ...
      await prisma.lesson.create({
        data: {
          ...
          // В демо-окружении видеоплатформы нет, и процент просмотра никогда
          // не вырастет. Порог снят, иначе урок нельзя отметить пройденным.
          // В рабочем курсе он задаётся в редакторе этапа.
          minWatchPercent: 0,
