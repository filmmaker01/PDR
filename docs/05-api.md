...
- Размерный класс выводится из габаритов, если они указаны: 40×40 см — это `XL` независимо от того, что прислал клиент.
...
- `POST /workspaces/:id/assessments/preview` считает по параметрам повреждения **без сохранения**: мастер крутит размеры и сразу видит сумму.
...
- `PATCH /workspaces/:id/assessments/:assessmentId` — ручная правка любых значений и итога.
