...
- `GET /workspaces/:id/analytics/summary?from=&to=&assigneeMemberId=` принимает даты как дни мастерской (`YYYY-MM-DD`) и возвращает завершённые заказы, их сумму, поступления, текущую задолженность, средний чек, новых клиентов и занятое время календаря.
