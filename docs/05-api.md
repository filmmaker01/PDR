...
| GET | `/workspaces/:id/today` | O, E | сводка «Сегодня»: активные заказы и готовые к выдаче (сотрудник — только свои) |
...
## Реализация аналитики (этап 13)

- `GET /workspaces/:id/analytics/summary?from=&to=&assigneeMemberId=` принимает даты как дни мастерской (`YYYY-MM-DD`) и возвращает завершённые заказы, их сумму, поступления, средний чек, новых клиентов и занятое время календаря.
- `GET .../analytics/series` добавляет `granularity` (`day` по умолчанию, `week`) и отдаёт точки с границами по часовому поясу мастерской.
- `GET .../analytics/employees` возвращает строки по исполнителям с именем, цветом и признаком активности; заказы без исполнителя приходят с `memberId: null`.
- Слишком длинный (больше 366 дней) и перевёрнутый период отвергаются с `422`.

## Реализация клуба (этап 14)
...
## Реализация обращений, схемы кузова и оценок (этап 17)

### Обращения

- `GET /workspaces/:id/leads` — список с фильтрами `status`, `source`, `channel`, `q`, `due` (только те, кому пора звонить) и `archived`.
- `GET /workspaces/:id/leads/summary` — счётчики для карточки на главной: открытые всего, разбивка по статусам и `due` (просроченные «перезвонить»).
- `POST /workspaces/:id/leads` создаёт обращение вместе с отмеченными на схеме повреждениями и уже загруженными снимками. Принимает `Idempotency-Key`. Ответ содержит `photos: { attached, failed }`: неудача на одной фотографии не отменяет обращение.
- `PATCH /workspaces/:id/leads/:leadId` — правка контакта, машины, канала, комментария и даты следующего контакта.
- `POST .../leads/:leadId/transition` — смена статуса. `rejected` требует причину, `callback` — дату следующего контакта, иначе `422`. Недопустимый переход — `409` со списком разрешённых.
- `POST .../leads/:leadId/schedule` записывает клиента в календарь: заводит клиента и автомобиль, если их ещё нет, и переводит обращение в «Записан».
- `POST .../leads/:leadId/convert` создаёт заказ и переносит в него повреждения, снимки и оценки. Ответ: `orderId`, `orderNumber`, `movedDamages`, `movedPhotos`. Повторная конверсия — `409`.
- `POST .../leads/:leadId/archive` — право `leads.manage` (владелец).

### Схема кузова и повреждения

- `GET /workspaces/:id/body-scheme` — справочник элементов кузова, типов повреждений и размерных классов для интерактивной схемы.
- `GET|POST /workspaces/:id/leads/:leadId/damages` и `GET|POST /workspaces/:id/orders/:orderId/damages` — симметричные маршруты: схема в интерфейсе одна, меняется только владелец карточки.
- `PATCH|DELETE /workspaces/:id/damages/:damageId`. Удаление снимает привязку со снимков и позиций оценок, но не удаляет их.
- Размерный класс выводится из габаритов, если они указаны: 40×40 см — это `40x40` независимо от того, что прислал клиент.
- Размерная сетка: мелкие вмятины `S`, `M`, `L`, `XL` (до 20 см) и зоны `20x40`, `40x40`, `40x60`, `60x60`, `60x100`, `100x100`. Класс выбирается по наибольшей стороне **и** площади сразу: только по стороне 40×60 и 60×60 неразличимы, только по площади узкая вмятина 10×60 мм попала бы в мелочь.

### Фотографии и разметка

- `POST /workspaces/:id/leads/:leadId/photos` — привязка загруженного файла к обращению (`scope: order_photo`, как и у заказов).
- `POST /workspaces/:id/photos/:photoId/markup` сохраняет разметку: `annotation` (`{v:1,shapes:[…]}` в нормированных координатах 0–1) и необязательный `annotationFileId` — сведённую картинку. `annotation: null` снимает разметку. Оригинал не перезаписывается; попытка указать в `annotationFileId` сам оригинал — `422`.
- `GET .../photos/:photoId/download` и `.../download/markup` — ссылки на оригинал и на снимок с разметкой.
- Прежние адреса `/orders/:orderId/photos/:photoId` сохранены, чтобы обновление backend не ломало открытые сессии Mini App.

### Оценка ремонта

- `GET /workspaces/:id/assessments/capabilities` — какие способы доступны в этой установке. `ai.available: false`, когда провайдер не настроен; интерфейс тогда прячет кнопку.
- `POST /workspaces/:id/assessments/preview` считает по параметрам повреждения **без сохранения**: мастер крутит размеры и сразу видит сумму. Принимает `items`, `extras` (арматурные работы) и `priceCoefficient`; возвращает `baseMinor`, `priceCoefficient`, `pdrMinor`, `extrasMinor`, `totalMinor` и готовую строку `formula`.
- `POST .../leads/:leadId/assessments/analyze` (и `.../orders/:orderId/...`) разбирает фотографии и возвращает предложение с подписью «Предварительная AI-оценка» и текстом отказа от ответственности. Ничего не сохраняет.
- `POST .../leads/:leadId/assessments` сохраняет оценку. Сервер всегда считает суммы сам по прайсу; присланный `totalMinor` принимается как **правка мастера** и помечает оценку `overridden`. `suggestedMinor` при этом сохраняется — видно, от чего отступили.
- `PATCH /workspaces/:id/assessments/:assessmentId` — ручная правка любых значений и итога. Правка одного коэффициента состав оценки не трогает.
- Без настроенного провайдера разбор фотографии отвечает `503` с кодом `ai_unavailable` и понятным текстом. Это не сбой: ручная оценка и расчёт по прайсу работают.

### Коэффициент стоимости и арматурные работы (этап 18)

- Расчёт идёт тремя стадиями: базовый расчёт по прайсу → коэффициент цены мастера → ручной итог. Базовый расчёт остаётся в `baseMinor` и виден всегда.
- Коэффициент — проценты шагом 5 в границах 50…200 (−50 %…+100 %). Значение вне границ или мимо шага — `422`. Границы отдаёт `GET .../assessments/capabilities` в поле `priceCoefficient`, чтобы интерфейс не придумывал их сам.
- Привычка мастерской живёт в настройке `default_price_coefficient` (`PATCH /workspaces/:id`). Новая оценка берёт её и разрешает изменить для конкретного случая.
- Арматурные работы — позиции того же прайса с видом не `damage`. `GET /workspaces/:id/price-list?kind=disassembly` отдаёт справочник. Позиция повреждения арматурной работой не становится: `422`.
- В оценке арматурные работы приходят в `extras: [{ priceListItemId | title, damageId?, quantity?, unitPriceMinor? }]`, сохраняются позициями оценки с `kind: disassembly` и суммируются с PDR. Коэффициент к ним не применяется.
- Позиция справочника, использованная в оценке, не удаляется, а скрывается — как и позиция, попавшая в смету.

### Документы заказа

- `GET /workspaces/:id/order-documents` — перечень печатных форм: `inspection_act` (акт осмотра автомобиля), `work_order` (заказ-наряд), `completion_act` (акт выполненных работ).
- `GET /workspaces/:id/orders/:orderId/documents/:kind/pdf` — печатная форма. По умолчанию отдаётся просмотром (`inline`), с `?download=true` — файлом. Неизвестный вид — `404`.
- Документ собирается из данных заказа: клиент, автомобиль с госномером и VIN, повреждения со схемы, работы и суммы, оплаты. Основание сумм — согласованная смета; пока её нет — последняя оценка вместе с коэффициентом и арматурными работами.
- Реквизиты берутся из настроек мастерской: `document_legal_name` и `document_requisites` (`PATCH /workspaces/:id`). Без них печатается название мастерской, телефон и адрес.

### Календарь месяцем

- `GET /workspaces/:id/appointments/month?month=YYYY-MM` — сколько записей в каждом дне месяца: `days: [{ day, total, active }]`. `active` не считает отменённые и неявки: индикатор в календаре означает работу, а не историю отмен. Дни считаются в часовом поясе мастерской.

### Выгрузка

Полная выгрузка мастерской дополнена `leads.csv` и `damages.csv`: обращение —
такая же часть базы, как заказ, и без него в архиве терялась бы половина
воронки. Снимки обращений складываются в папку `photos/lead-<номер>`.

### Убрано

- `GET /workspaces/:id/debts` и показатель `outstandingDebtMinor` в аналитике удалены: в PDR-мастерской работы оплачиваются сразу. Учёт оплат сохранён — остаток по заказу отдаёт `GET /workspaces/:id/orders/:orderId/payments` в поле `remainingMinor`.

### Размер повреждения и работы детали (этап 19)

- Повреждение отдаётся с тремя подписями сразу: `widthMm`/`heightMm` — как
  измерил мастер, `sizeText` — готовая строка «300 × 300 см», `zoneLabel` —
  тарифная зона («40×40» или «100×100+», если повреждение крупнее сетки),
  `zoneCapped` — признак такого превышения. Зона считает цену, размер идёт в
  документы; подменять одно другим нельзя.
- `POST|PATCH .../damages` принимают `extraWorks: [{ priceListItemId | title,
  quantity?, unitPriceMinor? }]` — набор целиком, как его видит мастер на
  экране. Сервер берёт название и цену из справочника, ручная цена перебивает.
  Позиция справочника для повреждений арматурной работой не становится: `422`.
- Ответ содержит `extraWorks`, `extrasMinor` и `totalMinor` — итог по детали
  (ремонт плюс её работы).
- Те же работы принимает массив `damages` при создании обращения и заказа:
  карточка повреждения в форме новой записи ничем не отличается от карточки
  в готовом заказе.
- `PUT|POST .../estimates/:id/items` принимают `widthMm`/`heightMm`, ответ
  отдаёт `sizeText`.
- `PUT /v1/files/local` — приём файла локальным хранилищем. Ссылка подписана
  этим методом; POST оставлен для интеграционных тестов.

### Разметка снимков в новом обращении (этап 20)

- `POST .../leads` принимает у каждого элемента `photos` ещё `annotation` и
  `annotationFileId` — разметку, сделанную до сохранения формы. Сохраняется
  тем же путём, что `POST .../photos/:id/markup`. В ответе
  `photos.markupFailed` — сколько снимков прикрепилось без разметки.

### Заказ сразу с повреждениями; ссылка на документ для клиента (этап 21)

- `POST .../orders` принимает `damages` и `photos` в той же форме, что
  `POST .../leads`: повреждения создаются в транзакции заказа (с арматурными
  работами), снимки прикрепляются следом со связью `damageIndex` и разметкой.
  Ошибка в любом повреждении отменяет весь заказ. Ответ содержит
  `photos: { attached, failed, markupFailed }`.
- `POST .../orders/:id/documents/:kind/share` — временная ссылка на документ
  (неделя). Выдаётся тому, кто может открыть заказ. Токен подписан HMAC
  (`SESSION_JWT_SECRET` с отдельным префиксом) и несёт мастерскую, заказ,
  вид документа и срок.
