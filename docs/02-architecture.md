...
| Разбор фото повреждения | Внешняя модель, понимающая изображения, за адаптером `DamageVisionProvider`; протокол — Chat Completions, поэтому подходит любой совместимый сервис | Смена модели — это две переменные окружения, а не переписывание модуля. По умолчанию способ выключен, и продукт работает без него |
...
│   │   │   │   ├── ai/           # DamageVisionProvider: разбор фото за адаптером
...
| AI-оценка по умолчанию выключена (`AI_PROVIDER=disabled`) | Внешний ключ не должен быть условием работы продукта. Обращения, схема кузова, разметка снимков, ручная оценка и расчёт по прайсу от него не зависят, а адаптер `disabled` отвечает понятным отказом вместо выдуманного результата. `mock` запрещён в production: его разбор выглядел бы как настоящая оценка |
| Предложение расчёта и утверждённый итог хранятся раздельно | Мастер правит любую сумму, и после правки должно оставаться видно, от чего он отступил. Иначе результат модели со временем становится неотличим от решения человека |
| Разметка повреждения хранится вектором отдельно от снимка | Оригинал нельзя перезаписывать: в спорной ситуации именно он доказывает исходное состояние детали. Сведённая картинка — производная, её можно перерисовать |
| Повреждение, снимок и оценка принадлежат либо обращению, либо заказу, а при конверсии меняют владельца | Копирование дало бы две расходящиеся версии одного повреждения; проверка «ровно один владелец» стоит в базе, а не только в сервисе |
