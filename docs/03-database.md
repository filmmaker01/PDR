...
## 16. Обращения, повреждения и оценки (этап 17)

### `leads` — обращение

Единый входящий лид независимо от того, откуда пришёл клиент: приехал лично,
написал в мессенджер, прислал фотографии, получил оценку на месте или
дистанционно. Одна сущность вместо трёх: данные обращения переезжают в заказ
при конверсии и не вводятся повторно.

| Поле | Смысл |
|------|-------|
| `number` | порядковый номер внутри мастерской, счётчик `workspaces.lead_seq` |
| `client_id`, `vehicle_id` | заполняются, когда клиент уже есть в базе |
| `contact_name`, `contact_phone`, `contact_extra` | холодный контакт: имя, телефон в E.164, ник или ссылка |
| `vehicle_make`…`vehicle_color` | машина, которой ещё нет в справочнике |
| `source` | `online` / `offline` |
| `channel` | `telegram`, `whatsapp`, `vk`, `call`, `in_person`, `other`; может быть неизвестен |
| `status` | `new` → `estimated` → `awaiting_decision` → `callback` → `scheduled` → `rejected` |
| `estimate_minor` | предварительная оценка: итог последней сохранённой оценки |
| `next_contact_at` | когда перезвонить; по нему собирается список «пора звонить» |
| `converted_order_id`, `converted_at` | заказ, в который превратилось обращение |

Клиент и автомобиль **не создаются** при обращении: половина входящих — это
«написали и спросили цену», и карточка на каждого замусорила бы справочник.
Они заводятся в момент, когда без них нельзя, — при записи в календарь или при
конверсии в заказ, из тех же полей, что ввёл мастер.

История статусов — `lead_status_history`, устроена как `order_status_history`.

### `damages` — повреждение на схеме кузова

Принадлежит **либо** обращению, **либо** заказу (CHECK `damages_one_parent`).
Хранит код детали из `BODY_PANELS`, тип повреждения, размеры в миллиметрах,
количество, материал, сложность доступа, признак ребра, стоимость и её
источник (`manual` / `params` / `ai`).

Одна деталь может быть повреждена несколько раз: повторный выбор элемента на
схеме создаёт ещё одну запись, а не перетирает прежнюю. Именно это позволяет
показать, какую из двух вмятин на двери согласовали в работу.

### `assessments` и `assessment_items` — оценка ремонта

Три способа: `manual` (мастер называет сумму), `params` (расчёт по прайсу от
параметров повреждения), `ai` (разбор фотографии заполняет параметры, после
чего работает тот же расчёт по прайсу).

Предложение и итог хранятся **раздельно**: `suggested_minor` — что посчитал
расчёт, `total_minor` — что утвердил мастер, `overridden` — правил ли он
итог руками. То же на уровне строки: `suggested_unit_price_minor` и
`unit_price_minor`. Результат AI ценой не является ни в одном состоянии;
`ai_provider`, `ai_model`, `ai_raw`, `ai_confidence` хранятся, чтобы спорную
оценку можно было перепроверить позже.

### `order_photos` — снимок мастерской

Таблица обобщена: `order_id` стал необязательным, добавлены `lead_id`
(снимок обращения), `damage_id` (привязка к конкретному повреждению),
`annotation` (векторная разметка), `annotation_file_id` и `annotated_at`.
Владелец ровно один — CHECK `order_photos_one_parent`.

**Оригинал никогда не перезаписывается.** Разметка живёт отдельно: вектор в
`annotation`, сведённая картинка — самостоятельным файлом. CHECK
`order_photos_annotation_not_original` запрещает подменить оригинал сведённой
картинкой. Благодаря этому в спорной ситуации видно и исходное состояние
детали, и ровно ту вмятину, которую согласовали в работу.

### Конверсия обращения в заказ

Одна транзакция: заводятся клиент и автомобиль (если их не было), создаётся
заказ, а повреждения, снимки и оценки **переезжают** в него `UPDATE`-ом —
не копируются. Запись в календаре получает `order_id`. Поэтому двух
расходящихся версий одного повреждения возникнуть не может.
...
| Повреждение, снимок и оценка принадлежат ровно одной карточке | CHECK `*_one_parent` (обращение либо заказ) |
| Разметка не подменяет оригинал снимка | CHECK `order_photos_annotation_not_original` + проверка в сервисе |
| Обращение превращается в заказ один раз | `converted_order_id` + CHECK `leads_converted_consistent` |
| Один файл не прикрепляется к обращению дважды | частичный уникальный индекс `order_photos_lead_id_file_id_key` |
...
`assessments` получила три поля:

| Поле | Смысл |
|---|---|
| `base_minor` | базовый расчёт PDR по прайсу **до** коэффициента; остаётся видимым всегда |
| `price_coefficient` | коэффициент цены в процентах, 50…200 шагом 5 (проверка в базе) |
| `extras_minor` | арматурные работы; коэффициент к ним не применяется |

Итог оценки — `price_coefficient` от `base_minor` плюс `extras_minor`, и
поверх этого `total_minor`, который мастер вправе назначить руками. Поэтому
хранятся все три: по одному итогу восстановить формулу нельзя, а показывать
её нужно и через полгода.

`assessment_items` получила `kind` (`estimate_item_kind`, по умолчанию
`damage`) и `title`. Арматурная работа — такая же позиция оценки, только не
про повреждение: второго механизма расчёта не появляется, и общий итог
складывается из обоих видов одним запросом. У позиции не про повреждение
название обязательно — проверка в базе, иначе в печатном документе была бы
пустая строка.

Справочник арматурных работ — это `price_list_items` с `kind <> 'damage'`.
Отдельной таблицы нет сознательно: у позиции те же название, цена, признак
активности и порядок, а разделение по виду уже есть в смете. Новая мастерская
получает стартовый набор позиций, иначе раздел в оценке открывался бы пустым.

Размерные классы расширены зонами: к `S`, `M`, `L`, `XL` добавились
`20x40`, `40x40`, `40x60`, `60x60`, `60x100`, `100x100`. Коды хранятся в
`size_class` у прайса, повреждений, позиций смет и оценок; старые значения
продолжают работать.

## Фактический размер и арматурные работы детали (этап 19)

`estimate_items` получила `width_mm` и `height_mm`. Размер повреждения и
тарифная зона — разные вещи: зона нужна калькулятору, а в документе клиент
должен видеть измеренное. Повреждение и позиция оценки габариты и так хранили,
позиция сметы — нет, и после согласования сметы размер терялся.

`damage_extra_works` — арматурные работы конкретного повреждения:

| Поле | Смысл |
|---|---|
| `damage_id` | чьи это работы; удаление повреждения уносит их с собой |
| `price_list_item_id` | позиция справочника, если работа выбрана из него |
| `title`, `quantity`, `unit_price_minor` | название и цена на момент добавления |

Работы живут у повреждения, а не только в оценке, потому что мастер добавляет
их там же, где описывает деталь, — до всякой оценки. Иначе их негде держать
между «отметил деталь» и «сделал оценку», а итог по детали («ремонт двери плюс
её разборка») не собрать ни в карточке, ни в документе. Справочник и расчёт при
этом общие с оценкой: цены берутся из `price_list_items` с видом не `damage`,
считает тот же `resolveExtra`.
