...
Побочные эффекты переходов: `in_progress` ставит `started_at`; `ready` — `ready_at`; `delivered` — `delivered_at`; `cancelled` — отменяет будущие записи заказа. Все переходы пишут `order_status_history` и `audit_log`.
...
- Остаток по заказу считается там, где его смотрят, — в карточке оплат (`remainingMinor`). Отдельного показателя задолженности нет: в PDR-мастерской работы оплачиваются сразу, и сводная цифра «сколько нам должны» только отвлекала.
